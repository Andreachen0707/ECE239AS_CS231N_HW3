...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    out_fc_1 = np.dot(X,W1.T)+b1
    out_rl = np.maximum(0,out_fc_1)
    scores = np.dot(out_rl,W2.T)+b2 
    ...
    loss += 0.5*reg*np.sum(W1.T**2) + 0.5*reg*np.sum(W2.T**2)
    dx = probs.copy()
    dx[np.arange(N), y] -= 1
    dx /= N
    ...
    dimension = 1
    X_cal = out_rl
    for i in range(1,len(out_rl.shape)):
      dimension *=out_rl.shape[i]
    X_cal = X_cal.reshape(out_rl.shape[0],dimension)
    dX_2 = np.dot(dx,W2).reshape(out_rl.shape)
    grads['b2'] = np.sum(dx,axis = 0)
    grads['W2'] = np.dot(X_cal.T,dx)
    out_1 = out_fc_1
    out_1[out_1>0] = 1
    out_1[out_1<0] = 0
    dX_1 = dX_2*out_1
    ...
    for i in range(1,len(X.shape)):
      ...
      dX_0 = np.dot(dX_1,W1).reshape(X.shape)
      grads['b1'] = np.sum(dX_1,axis = 0)
      grads['W1'] = np.dot(X_cal.T,dX_1)
